document.addEventListener("DOMContentLoaded", () => {
  ...
  function initializeApp() {
    ...
    const messageperContainer = document.getElementById("hero-user-message");
    ...
    auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        navAuthContainer.innerHTML = `
                ...
                `;
        messageperContainer.textContent = `Bienvenue, ${prenom} ! Découvrez notre collection exclusive de chaises et meubles design, fabriqués avec des matériaux de qualité supérieure.`;
        navAdminContainer.style.display = "none"; // Cacher le menu admin par défaut
  ...
  // Affiche le nom de l'utilisateur dans le paragraphe hero si connecté
  firebase.auth().onAuthStateChanged(function (user) {
    const heroMsg = document.getElementById("hero-user-message");
    if (user && heroMsg) {
      // Cherche le nom dans Firestore
      firebase
        .firestore()
        .collection("users")
        .doc(user.uid)
        .get()
        .then(function (doc) {
          let nom = doc.exists && doc.data().nom ? doc.data().nom : null;
          if (nom) {
            heroMsg.textContent = `Bienvenue, ${nom} ! Découvrez notre collection exclusive de chaises et meubles design, fabriqués avec des matériaux de qualité supérieure.`;
          }
        });
    } else if (heroMsg) {
      // Si pas connecté, texte par défaut
      heroMsg.textContent =
        "Découvrez notre collection exclusive de chaises et meubles design, fabriqués avec des matériaux de qualité supérieure.";
    }
  });
